...
export function getCurrentPlayName(info: any) {
  const lang = getLocal('lang') ? Number(getLocal('lang')) : 1
  return info[`name_${langNameKey[lang]}`] || ''
}

export function getCurrentPointName(info: any) {
  const lang = getLocal('lang') ? Number(getLocal('lang')) : 1
  return info[`desc_${langNameKey[lang]}`] || ''
}

export function getCurrentTeam1Name(info: any) {
  const lang = getLocal('lang') ? Number(getLocal('lang')) : 1
  return info[`team_name_1_${langNameKey[lang]}`] || ''
}

export function getCurrentTeam2Name(info: any) {
  const lang = getLocal('lang') ? Number(getLocal('lang')) : 1
  return info[`team_name_2_${langNameKey[lang]}`] || ''
}

export function getCurrentEventName(info: any) {
  const lang = getLocal('lang') ? Number(getLocal('lang')) : 1
  return info[`event_name_${langNameKey[lang]}`] || ''
}

export function getCurrentGameName(info: any) {
  const lang = getLocal('lang') ? Number(getLocal('lang')) : 1
  return info[`game_name_${langNameKey[lang]}`] || ''
}

export function getCurrentStageName(info: any) {
  const lang = getLocal('lang') ? Number(getLocal('lang')) : 1
  return info[`stage_name_${langNameKey[lang]}`] || ''
}
