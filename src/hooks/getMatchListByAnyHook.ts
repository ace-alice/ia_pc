 ...
 **/
export default function getMatchListByAnyHook(handicap: HandicapType, Func?: Function | any) {
  ...
  function toUpdateList(newList: MatchListType) {
    const newListSet = newList.filter((match) => {
      const inx = matchList.value.findIndex(matchL => +matchL.id === +match.id)
      return inx === -1
    })
    matchList.value = [...newListSet, ...matchList.value].sort(
      (a: any, b: any) => {
  ...
  function getList() {
    return new Promise((resolve) => {
      if (isFinish.value || !canLoadingMore.value) {
        resolve(true)
        return
      }
      loadList.value = true
      ...
      //   form.is_resul = true
      // }
      if (+currentGameId.value && handicap !== 'hot') {
  ...
  watch(
    () => [currentDate.value, checkedEvenList.value.length, currentGameId.value],
    () => {
      if (handicap === 'hot') {
        return
      }
